...
public class MvtTileDecoder extends PbfDecoder {
    private static final Logger log = LoggerFactory.getLogger(MvtTileDecoder.class);

    // Not sure why this is 15?
    private static final int TAG_TILE_LAYERS = 15;
    ...
    public boolean decode(Tile tile, ITileDataSink mapDataCallback, InputStream is)
        ...
        if (debug)
            System.out.println(tile + " decode");

        System.out.println("New MVT Decoder used!");
        ...
        try{
            ...
            System.out.println("Found "+jtsMvt.getLayers().size()+ " layers");
    ...
    private boolean decodeLayer() throws IOException {
        ...
        if (debug)
            System.out.println("add layer " + name);
